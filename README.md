...
# Custom Hadoop + Spark + Hudi Docker Lab
## Start
...
Enter the Hudi dev container:

```bash
docker compose exec hudi-dev bash
```

Initialize a few HDFS directories once:
...
```bash
cd /workspace/hudi
mvn -DskipTests -Dspark3.5 -Dscala-2.12 -pl packaging/hudi-spark-bundle -am package
```
